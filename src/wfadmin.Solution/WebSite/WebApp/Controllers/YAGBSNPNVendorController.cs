...
{
  public class YAGBSNPNVendorController : Controller
  {
    private readonly NLog.ILogger logger;
    private readonly SqlSugar.ISqlSugarClient db;
    public YAGBSNPNVendorController(NLog.ILogger logger, SqlSugar.ISqlSugarClient db)
    {
      this.logger = logger;
      this.db = db;
    ...
    {
      var filters = PredicateBuilder.From<YAGBSNPNVendor>(filterRules);
      var count =await this.db.Queryable<YAGBSNPNVendor>().Where(filters)
         .CountAsync();
      var result=await this.db.Queryable<YAGBSNPNVendor>().Where(filters)
        .OrderBy($"{sort} {order}")
    ...
    public async Task<JsonResult> Sync(string vendorcode) {
      try
      {
        var result = await Task.Run(() =>
        {
          ...
          return sync.UpdateVendorForGBS(vendorcode);
        });
        this.logger.Info($"sync&update gbs vendor completed,{string.Join(",", result)}");
