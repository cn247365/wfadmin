...
{
  public class YAVHSSalesOrderController : Controller
  {
    private readonly NLog.ILogger logger;
    private readonly SqlSugar.ISqlSugarClient db;
    public YAVHSSalesOrderController(NLog.ILogger logger, SqlSugar.ISqlSugarClient db)
    {
      this.logger = logger;
      this.db = db;
    ...
    {
      var filters = PredicateBuilder.From<YAVHSSalesOrder>(filterRules);
      var count =await this.db.Queryable<YAVHSSalesOrder>().Where(filters)
         .CountAsync();
      var result=await this.db.Queryable<YAVHSSalesOrder>().Where(filters)
        .OrderBy($"{sort} {order}")
    ...
    public async Task<JsonResult> Sync(string so) {
      try
      {
        var result = await Task.Run(() =>
        {
          ...
          return sync.UpdateSOForVHS(so);
        });
        this.logger.Info($"sync vhs So completed,{string.Join(",", result)}");
