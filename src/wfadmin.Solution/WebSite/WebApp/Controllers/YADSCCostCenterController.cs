...
{
  [Authorize]
  public class YADSCCostCenterController : Controller
  {
    private readonly NLog.ILogger logger;
    private readonly SqlSugar.ISqlSugarClient db;
    public YADSCCostCenterController(NLog.ILogger logger, SqlSugar.ISqlSugarClient db)
    {
      this.logger = logger;
      this.db = db;
    ...
    {
      var filters = PredicateBuilder.From<YADSCCostCenter>(filterRules);
      var count =await this.db.Queryable<YADSCCostCenter>().Where(filters)
         .CountAsync();
      var result=await this.db.Queryable<YADSCCostCenter>().Where(filters)
        .OrderBy($"{sort} {order}")
    ...
    public async Task<JsonResult> AcceptChanges(YADSCCostCenter[] yadsccostcenters)
    {
      try
      {
        //await this.db.Updateable<YADSCCostCenter>(yadsccostcenters).ExecuteCommandAsync();
        foreach (var item in yadsccostcenters)
        {
          if(item.TrackingState== TrackableEntities.TrackingState.Added)
          {
           await this.db.Insertable<YADSCCostCenter>(item).ExecuteCommandAsync();
          }else
            if ( item.TrackingState == TrackableEntities.TrackingState.Modified )
          {
            await this.db.Updateable<YADSCCostCenter>(item).ExecuteCommandAsync();
          } else if( item.TrackingState == TrackableEntities.TrackingState.Deleted )
          {
            await this.db.Deleteable<YADSCCostCenter>(item).ExecuteCommandAsync();
          } 
        }
    ...
    public async Task<JsonResult> DeleteChecked(int[] id)
    {
      try
      {
        await this.db.Deleteable<YADSCCostCenter>().In(id).ExecuteCommandAsync();
