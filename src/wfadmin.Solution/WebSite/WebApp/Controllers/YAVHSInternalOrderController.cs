...
{
  public class YAVHSInternalOrderController : Controller
  {
    private readonly NLog.ILogger logger;
    private readonly SqlSugar.ISqlSugarClient db;
    public YAVHSInternalOrderController(NLog.ILogger logger, SqlSugar.ISqlSugarClient db)
    {
      this.logger = logger;
      this.db = db;
    ...
    {
      var filters = PredicateBuilder.From<YAVHSInternalOrder>(filterRules);
      var count =await this.db.Queryable<YAVHSInternalOrder>().Where(filters)
         .CountAsync();
      var result=await this.db.Queryable<YAVHSInternalOrder>().Where(filters)
        .OrderBy($"{sort} {order}")
    ...
    public async Task<JsonResult> Sync(string order) {
      try
      {
        var result = await Task.Run(() =>
        {
          ...
          return sync.UpdateInternalOrderForVHS(order);
        });
        this.logger.Info($"sync vhs Internal Order completed,{string.Join(",", result)}");
