...
using VPCVendorSAPtoWorkflow;
using WebApp.Models.WfModel;

namespace WebApp.Controllers
{
  public class YAVPCVendorController : Controller
  {
    private readonly NLog.ILogger logger;
    private readonly SqlSugar.ISqlSugarClient db;
    public YAVPCVendorController(NLog.ILogger logger, SqlSugar.ISqlSugarClient db)
    {
      this.logger = logger;
      this.db = db;
    ...
    {
      var filters = PredicateBuilder.From<YAVPCVendor>(filterRules);
      var count =await this.db.Queryable<YAVPCVendor>().Where(filters)
         .CountAsync();
      var result=await this.db.Queryable<YAVPCVendor>().Where(filters)
        .OrderBy($"{sort} {order}")
    ...
    [HttpGet]
    public async Task<JsonResult> Sync(string vendorcode) {
      try
      {
        var result = await Task.Run(() =>
        {
          ...
          return sync.UpdateVendor(vendorcode);
        });
        this.logger.Info($"sync&update vpc vendor completed,{string.Join(",", result)}");
