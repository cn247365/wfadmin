...
{
  public class YAVPCVendorApproverController : Controller
  {
    private readonly NLog.ILogger logger;
    private readonly SqlSugar.ISqlSugarClient db;
    public YAVPCVendorApproverController(NLog.ILogger logger, SqlSugar.ISqlSugarClient db)
    {
      this.logger = logger;
      this.db = db;
    ...
    [HttpPost]
    public async Task<JsonResult> AcceptChanges(YAVPCVendorApprover[] rolelist)
    {
      try
      {
        //await this.db.Updateable<YAVPCVendorApprover>(yavpccostcenter).ExecuteCommandAsync();
        foreach (var item in rolelist)
        {
          if (item.TrackingState == TrackableEntities.TrackingState.Added)
          {
            await this.db.Insertable<YAVPCVendorApprover>(item).ExecuteCommandAsync();
          }
          else
            if (item.TrackingState == TrackableEntities.TrackingState.Modified)
          {
            await this.db.Updateable<YAVPCVendorApprover>(item).ExecuteCommandAsync();
          }
          else if (item.TrackingState == TrackableEntities.TrackingState.Deleted)
          {
            await this.db.Deleteable<YAVPCVendorApprover>(item).ExecuteCommandAsync();
          }
        }
    ...
    public async Task<JsonResult> DeleteChecked(int[] id)
    {
      try
      {
        await this.db.Deleteable<YAVPCVendorApprover>().In(id).ExecuteCommandAsync();
