...
{
  public class YAVHSWBSNoController : Controller
  {
    private readonly NLog.ILogger logger;
    private readonly SqlSugar.ISqlSugarClient db;
    public YAVHSWBSNoController(NLog.ILogger logger, SqlSugar.ISqlSugarClient db)
    {
      this.logger = logger;
      this.db = db;
    ...
    {
      var filters = PredicateBuilder.From<YAVHSWBSNo>(filterRules);
      var count =await this.db.Queryable<YAVHSWBSNo>().Where(filters)
         .CountAsync();
      var result=await this.db.Queryable<YAVHSWBSNo>().Where(filters)
        .OrderBy($"{sort} {order}")
    ...
    public async Task<JsonResult> Sync(string wbsno) {
      try
      {
        var result = await Task.Run(() =>
        {
          ...
          return sync.UpdateWBSForVHS(wbsno);
        });
        this.logger.Info($"sync vhs wbs no completed,{string.Join(",", result)}");
