...
public class DBHelper
{
    ...
    //protected static string ConnectionString1 = ConfigurationManager.AppSettings["ConnectionString1"];
    //public DBHelper()
    //{
    //}
    #region 公用方法
    ...
    public static decimal GetMaxID(string FieldName, string TableName, string ConnectionString)
    {
    ...
    public static bool ExecuteSqlTran(ArrayList SQLStringList, string ConnectionString)
    {
        int tmp = 0;
    ...
    /// <summary>
    /// 将Dataset类型转换为IList<>类型
    /// </summary>
    /// <typeparam name="T"></typeparam>
    /// <param name="sql">查询语句</param>
    /// <returns></returns>
    public static List<T> Query<T>(string sql, string ConnectionString) where T : new()
    {
        DataSet ds = Query(sql, ConnectionString);
        ...
        {
            T model = new T();
            UpdateModel<T>(model, row);
            list.Add(model);
        }
        return list;
    }

    private static void UpdateModel<T>(T model, DataRow row) where T : new()
    {
        //利用反射，给model中的属性赋值 
        foreach (DataColumn col in row.Table.Columns)
        {

            //找到与col.ColumnName 同名的属性对象
            ...
            if (row[col.ColumnName] == DBNull.Value)
                property.SetValue(model, default(T), null);
            else
